...
class WooProcessImportExport(models.TransientModel):
    ...
    def import_specific_product(self):
        ...
        for template_id in list(self.woo_template_ids.split(",")):
            ...
            attribute_data = results[0].get('attributes')
            attribute_ids_list += list(map(lambda attribute:attribute.get('id'),attribute_data))

        self.env['woo.product.template.ept'].with_context(
            specific_product_attribute_ids=list(set(attribute_ids_list))).sync_woo_attribute(self.woo_instance_id)
    ...
    def prepare_data_for_inventory_adjustment(self, woo_products, res_product, duplicate_woo_product, products_stock,
        ...
        if product:
            if (res_product.get('manage_stock') and
                    ...
                    product.product_id.type == 'consu' and product.product_id.is_storable == True):
