...
class Webhook(http.Controller):
    ...
    def delete_product_webhook(self):
        ...
        woo_template = request.env[woo_template_ref].sudo().search([("woo_tmpl_id", "=", res.get('id')),
    ...
    def restore_product_webhook(self):
        ...
        if not res:
            return False
        woo_template = request.env[woo_template_ref].with_context(active_test=False).search(
    ...
    def product_webhook_process(self):
        ...
        woo_template = request.env[woo_template_ref].sudo().with_context(active_test=False).search(
