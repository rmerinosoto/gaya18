...
class WooCouponDataQueueLineEpt(models.Model):
    ...
    def process_coupon_queue_line(self):
        ...
        start = time.time()
        qry = """UPDATE woo_coupon_data_queue_ept SET is_process_queue = %s WHERE is_process_queue = %s"""
    ...
    def auto_coupon_queue_lines_process(self):
        ...
        query = """SELECT coupon_data_queue_id FROM woo_coupon_data_queue_line_ept WHERE state = %s ORDER BY
        "create_date" ASC limit 1;"""
        self.env.cr.execute(query, ('draft',))
