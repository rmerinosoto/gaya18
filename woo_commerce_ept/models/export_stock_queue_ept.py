...
export_stock_queue_line_ref = "woo.export.stock.queue.line.ept"
...
class WooExportStockQueueEpt(models.Model):
    ...
    export_stock_queue_line_ids = fields.One2many(export_stock_queue_line_ref, "export_stock_queue_id")
    ...
    def create_queue_lines(self, stock_data):
        ...
        """
        vals_list = []
        queue_line_obj = self.env[export_stock_queue_line_ref]
        for data in stock_data:
            batch_details = data.get("batch_details")
            for batch_detail in batch_details:
                if isinstance(batch_detail, list):
    ...
    def update_val_list(self, variant_data, vals_list, final_data):
        product_id = self.env['woo.product.product.ept'].search(
            [('variant_id', '=', variant_data.get('id')), ('woo_instance_id', '=', self.woo_instance_id.id)])
        vals_list.append({
            "batch_details": [variant_data],
    ...
    def retrieve_dashboard(self, *args, **kwargs):
        ...
        return dashboard.get_data(table=export_stock_queue_line_ref)
