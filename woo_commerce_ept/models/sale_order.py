...
bus_bus_ref = 'bus.bus'
order_connection_error_msg = ("Something went wrong while importing Orders.\n\nPlease Check your Connection "
                               "and Instance Configuration.\n\n")
common_log_lines_ref = 'common.log.lines.ept'
sale_order_ref = 'sale.order'
...
class SaleOrder(models.Model):
    ...
    woo_instance_id = fields.Many2one(woo_instance_ref, "Woo Instance", copy=False)
    ...
    def create_woo_order_data_queue(self, woo_instance, orders_data, order_type, created_by="import"):
        ...
        bus_bus_obj = self.env[bus_bus_ref]
        while orders_data:
    ...
    def get_order_data_from_specific_ids(self, params, woo_instance, order_ids):
        ...
        for order_id in list(order_ids.split(",")):
            try:
                ...
                raise UserError(_(order_connection_error_msg + str(error)))
    ...
    def create_woo_log_lines(self, message, instance, queue_line=None, operation_type="import"):
        ...
        return self.env[common_log_lines_ref].create_common_log_line_ept(
    ...
    def create_woo_orders(self, queue_lines):
        ...
        for queue_line in queue_lines:
            ...
            try:
                ...
                _logger.info(message)
                self.env[common_log_lines_ref].create_common_log_line_ept(wpp_instance_id=woo_instance.id,
                                                                            message=message,
                                                                            module="woocommerce_ept",
                                                                            model_name=sale_order_ref,
    ...
    def cancel_in_woo(self):
        ...
        context.update({'active_model': sale_order_ref, 'active_id': self.id, 'active_ids': self.ids})
        return {
    ...
    def woo_change_shipping_partner(self, order_data, woo_instance, queue_line):
        ...
        if message and (need_update_partner or need_update_invoice_partner or need_update_shipping_partner):
            common_log_line_obj.create_common_log_line_ept(wpp_instance_id=woo_instance.id,
                                                            message=message,
                                                            module="woocommerce_ept",
                                                            model_name=sale_order_ref,
                                                            order_ref=order_data.get('name'),
        ...
        if response_order_line_ids:
            ...
            for need_to_remove_line in need_to_remove_lines:
                if need_to_remove_line.qty_delivered == 0:
                    ...
                    self.env[common_log_lines_ref].create_common_log_line_ept(woo_instance_id=instance.id,
                                                                                module="woocommerce_ept",
                                                                                message=message,
    ...
    def woo_create_schedule_activity_against_logline(self, log_lines, instance):
        ...
        model_id = ir_model_obj.sudo().search([('model', '=', common_log_lines_ref)])
