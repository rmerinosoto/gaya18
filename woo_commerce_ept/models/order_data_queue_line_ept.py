...
order_data_queue_ref = 'woo.order.data.queue.ept'
...
class WooOrderDataQueueLineEpt(models.Model):
    ...
    order_data_queue_id = fields.Many2one(order_data_queue_ref, ondelete="cascade")
    ...
    sale_order_id = fields.Many2one(sale_order_ref, copy=False,
    ...
    def auto_order_queue_lines_process(self):
        ...
        """
        order_queue_ids = []
        woo_order_data_queue_obj = self.env[order_data_queue_ref]
        start = time.time()
    ...
    def create_order_queue_schedule_activity(self, queue_id):
        ...
        model_id = ir_model_obj.search([("model", "=", order_data_queue_ref)])
    ...
    def process_order_queue_line(self, update_order=False):
        ...
        sale_order_obj = self.env[sale_order_ref]
        start = time.time()
