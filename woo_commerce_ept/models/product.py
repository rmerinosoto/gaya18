...
woo_product_ref = 'woo.product.product.ept'

class ProductProduct(models.Model):
    _inherit = 'product.product'

    def _compute_woo_product_count(self):
        woo_product_obj = self.env[woo_product_ref]
        for product in self:
    ...
    def write(self, vals):
        ...
        if 'active' in vals.keys():
            woo_product_product_obj = self.env[woo_product_ref]
            for product in self:
...
class Pricelist(models.Model):
    ...
    def _get_applicable_rules(self, products, date, **kwargs):
        ...
        woo_products = self.env[woo_product_ref].sudo().search([('product_id', 'in', products.ids)])
        if woo_products:
