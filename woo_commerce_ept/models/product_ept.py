...
woo_product_template_ept = "woo.product.template.ept"
woo_product_product_ept = "woo.product.product.ept"
product_template_ref = 'product.template'
woo_product_image_ref = 'woo.product.image.ept'
woo_product_attribute_ref = 'woo.product.attribute.ept'
common_log_lines_ref = 'common.log.lines.ept'
product_product_ref = 'product.product'
product_variation_batch = 'products/%s/variations/batch'
woo_attributes = 'WooCommerce Attributes'
import_product = 'Import Product'
product_id_ref = 'products/%s'
product_attribute_ref = 'product.attribute'
config_parameter_ref = 'ir.config_parameter'
sales_descritption_ref = 'woo_commerce_ept.set_sales_description'
process_failed_log_msg = 'Process Failed of Product %s||Queue %s||Reason is %s'
date_time_format = '%Y-%m-%dT%H:%M:%S'
update_order_ref = 'Update Product \n%s'
product_pricelist_item_ref = 'product.pricelist.item'


class WooProductTemplateEpt(models.Model):
    _name = woo_product_template_ept
    ...
    def _compute_total_sync_variants(self):
        ...
        """
        woo_product_obj = self.env[woo_product_product_ept]
    ...
    product_tmpl_id = fields.Many2one(product_template_ref, "Product Template", required=True, ondelete="cascade")
    ...
    woo_product_ids = fields.One2many(woo_product_product_ept, "woo_template_id", "Products")
    ...
    woo_image_ids = fields.One2many(woo_product_image_ref, "woo_template_id")
    ...
    def write(self, vals):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        woo_product_product_obj = self.env[woo_product_product_ept]
    ...
    def get_variant_data(self, variant, instance, update_image):
        ...
        """
        attrs = []
        woo_attribute_obj = self.env[woo_product_attribute_ref]
        variation_data = {}
        attr_data = {}
    ...
    def woo_create_queue_for_export_stock(self, instance, woo_templates):
        ...
        Task Id : 199065
        """
        common_log_line_obj = self.env[common_log_lines_ref]
    ...
    def check_stock_type(self, instance, product_ids):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        prod_obj = self.env[product_product_ref]
        warehouse = instance.woo_stock_export_warehouse_ids
        ...
        for template in woo_variable_products:
            ...
            if variations:
                ...
                for woo_variants in variant_batches:
                    ...
                    try:
                        res = wc_api.post(product_variation_batch % template.woo_tmpl_id,
                                          {'update': woo_variants})
                    ...
                    _logger.info('Completed Variations Batch Stock Update Process Completed with [status: %s]',
                                 res.status_code)
                    log_line = self.check_woocommerce_response(res, "Update Product stock", woo_product_product_ept,
                                                               instance)
    ...
    def check_woocommerce_response(self, response, process, model_name, instance, product_template=False,
        ...
        """
        common_log_line_obj = self.env[common_log_lines_ref]
        if process in [woo_attributes, import_product]:
            operation = "import"
        else:
            operation = "export"
            ...
            if batch_update_data:
                ...
                if response.get('data') and response.get('data', {}).get('status') != 200:
                    log_line = common_log_line_obj.create_common_log_line_ept(operation_type="export",
                                                                              module="woocommerce_ept",
                                                                              woo_instance_id=instance.id,
                                                                              model_name=woo_product_product_ept,
    ...
    def woo_unpublished(self):
        ...
        if self.woo_tmpl_id:
            try:
                res = wc_api.put(product_id_ref % self.woo_tmpl_id, {'status': 'draft'})
            except Exception as error:
                raise UserError(_("Something went wrong while updating Product.\n\nPlease Check your Connection and "
                                  "Instance Configuration.\n\n" + str(error)))

            response = self.check_woocommerce_response(res, "Unpublish Product", woo_product_template_ept, instance)
            ...
            if response.get('data') and response.get('data', {}).get('status') not in [200, 201]:
                ...
                common_log_line_obj.create_common_log_line_ept(operation_type="export", module="woocommerce_ept",
                                                               ...
                                                               model_name=woo_product_template_ept)
            else:
    ...
    def woo_published(self):
        ...
        if self.woo_tmpl_id:
            try:
                res = wc_api.put(product_id_ref % self.woo_tmpl_id, {'status': 'publish'})
            except Exception as error:
                raise UserError(_("Something went wrong while updating Product.\n\nPlease Check your Connection and "
                                  "Instance Configuration.\n\n" + str(error)))

            response = self.check_woocommerce_response(res, "Publish Product", woo_product_template_ept, instance)
            if not isinstance(response, (dict, list)):
                return False
            if response.get('data', {}) and response.get('data', {}).get('status') not in [200, 201]:
                message = response.get('message')
    ...
    def sync_woo_attribute_term(self, instance, attribute_ids=False):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        obj_woo_attribute = self.env[woo_product_attribute_ref]
    ...
    def woo_import_all_attributes(self, wc_api, response, instance):
        ...
        if int(total_pages) >= 2:
            for page in range(2, int(total_pages) + 1):
                ...
                data = self.check_woocommerce_response(res, woo_attributes, woo_product_attribute_ref,
                                                       instance)
    ...
    def find_or_create_woo_attribute(self, attributes_data, instance):
        ...
        """
        obj_woo_attribute = self.env[woo_product_attribute_ref]
        odoo_attribute_obj = self.env[product_attribute_ref]
    ...
    def sync_woo_attribute(self, instance):
        ...
        attributes_data = self.check_woocommerce_response(response, woo_attributes,
                                                          woo_product_attribute_ref, instance)
    ...
    def import_all_woo_products(self, instance, page, from_date, to_date):
        ...
        response = self.check_woocommerce_response(res, import_product, woo_product_template_ept, instance)
    ...
    def get_templates_from_woo(self, instance, from_date, to_date, template_id):
        ...
        try:
            if template_id:
                res = wc_api.get(product_id_ref % template_id)
            else:
    ...
    def search_odoo_product_variant(self, woo_instance, product_sku, variant_id):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        odoo_product = self.env[product_product_ref]
        woo_product_obj = self.env[woo_product_product_ept]
    ...
    def prepare_woo_attribute_line_vals(self, attributes_data):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        product_attribute_obj = self.env[product_attribute_ref]
    ...
    def woo_create_variant_product(self, product_template_dict, woo_instance):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        ir_config_parameter_obj = self.env[config_parameter_ref]
        product_template_obj = self.env[product_template_ref]
        ...
        if attrib_line_vals:
            ...
            if ir_config_parameter_obj.sudo().get_param(sales_descritption_ref):
        ...
        for variation_attribute in variation_attributes:
            ...
            for attribute in template_attributes:
                if attribute.get('variation') and \
                    ...
                    break
            product_attribute = self.env[product_attribute_ref].get_attribute(attribute_name, "radio", "always", True)
    ...
    def woo_set_variant_sku(self, woo_instance, product_template_dict, product_template, sync_price_with_product=False):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        odoo_product_obj = self.env[product_product_ref]
    ...
    def find_or_create_common_product_image(self, woo_template, image, url, product_dict={}, woo_product=False):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        woo_product_image_obj = self.env[woo_product_image_ref]
    ...
    def update_woo_template_images(self, template_images, woo_template):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        woo_product_image_obj = woo_product_images = self.env[woo_product_image_ref]
    ...
    def update_product_images(self, template_images, variant_image, woo_template, woo_product, template_image_updated,
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        woo_product_image_obj = need_to_remove = self.env[woo_product_image_ref]
    ...
    def sync_products(self, product_data_queue_lines, woo_instance, skip_existing_products=False,
        ...
        queue_counter = 0
        # if order_queue_line:
        #     self.env[woo_product_template_ept].sync_woo_attribute(woo_instance)
        # else:
        ...
        for product_data_queue_line in product_data_queue_lines:
            ...
            if data.get('type') not in ['simple', 'variable', 'bundle', 'grouped', 'external']:
                ...
                _logger.info(process_failed_log_msg, woo_product_template_id,
    ...
    def template_attribute_process(self, woo_instance, odoo_template, variant, template_title, data,
        ...
        if odoo_template.attribute_line_ids:
            ...
            for attribute in variant.get("attributes"):
                attribute = self.env[product_attribute_ref].get_attribute(attribute["name"])
            ...
            if not template_attribute_value_domain:
                for woo_attribute in variant.get("attributes"):
                    attribute_id = self.env[product_attribute_ref].get_attribute(woo_attribute["name"], auto_create=True)
                ...
                template_attribute_value_domain = self.find_template_attribute_values(data.get("attributes"),
                                                                                      variant.get("attributes"),
                                                                                      odoo_template)
            odoo_product = self.env[product_product_ref].search(template_attribute_value_domain)
        ...
        if self.env[config_parameter_ref].sudo().get_param(sales_descritption_ref):
            template_vals.update({"description_sale": variant.get("description", "")})

        odoo_product = self.env[product_product_ref].create(template_vals)
        return odoo_product
        ...
        for variant in product_response["variations"]:
            ...
            if not woo_product:
                ...
                variant_info.update({"product_id": odoo_product.id,
                                     "woo_template_id": woo_template.id})
                woo_product = self.env[woo_product_product_ept].create(variant_info)
            else:
            ...
            if update_price:
                ...
                if woo_instance.woo_extra_pricelist_id:
                    ...
                    pricelist_item.date_start = datetime.strptime(variant.get('date_on_sale_from'),
                                                                  date_time_format) if variant.get(
        ...
        if not woo_product:
            if not woo_template:
                if not odoo_template and woo_instance.auto_import_product:
                    ...
                    odoo_template = self.env[product_template_ref].create(template_vals)
        ...
        if update_price:
            ...
            if woo_instance.woo_extra_pricelist_id:
                ...
                pricelist_item.date_start = datetime.strptime(product_response.get('date_on_sale_from'),
                                                              date_time_format) if product_response.get(
                    'date_on_sale_from') else False
        ...
        for templates in batches:
            ...
            if batch_update_data:
                ...
                if not isinstance(response, dict):
                    continue
                if response.get('data', {}) and response.get('data', {}).get('status') != 200:
                    message = update_order_ref % (response.get('message'))
                    common_log_line_obj.create_common_log_line_ept(operation_type="export", module="woocommerce_ept",
                ...
                for product in response.get("update"):
                    if product.get("error"):
                        message = update_order_ref % (product.get("error").get('message'))
                        common_log_line_obj.create_common_log_line_ept(
    ...
    def check_available_products_in_woocommerce(self, wc_api, instance):
        ...
        woo_template_ids = self.env[woo_product_template_ept].search(
    ...
    def update_stock(self, instance, export_stock_from_date):
        ...
        Migrated Maulik Barad on Date 07-Oct-2021.
        """
        product_obj = self.env[product_product_ref]
        woo_product_product_obj = self.env[woo_product_product_ept]
    ...
    def export_product_attributes_in_woo(self, instance, attribute):
        ...
        if attribute_res.status_code == 400:
            ...
            if not woo_attribute:
                odoo_attributes = self.env[product_attribute_ref].search([('name', '=', attribute.name)])
                woo_attribute = obj_woo_attribute.search(
    ...
    def prepare_product_variant_dict(self, instance, template, data, basic_detail, update_price, update_image):
            ...
            if update_price:
                ...
                if instance.woo_extra_pricelist_id:
                    ...
                    if sale_price_rule:
                        ...
                        date_on_sale_from = self.env[product_pricelist_item_ref].browse(sale_price_rule).date_start
                        date_on_sale_to = self.env[product_pricelist_item_ref].browse(sale_price_rule).date_end
                        info.update({'date_on_sale_from': date_on_sale_from.strftime(
                            date_time_format) if date_on_sale_from else "",
                                     'date_on_sale_to': date_on_sale_to.strftime(
                                         date_time_format) if date_on_sale_to else ""})
        ...
        if data.get('variations'):
            ...
            for woo_variants in variant_batches:
                ...
                try:
                    res = wc_api.post(product_variation_batch % (data.get('id')), {'update': woo_variants})
                except Exception as error:
        ...
        if variants_to_create:
            ...
            try:
                wc_api.put(product_id_ref % (data.get("id")), data)
            except Exception as error:
                raise UserError(_("Something went wrong while Updating product.\n\nPlease Check your Connection and "
                                  "Instance Configuration.\n\n" + str(error)))
            ...
            try:
                res = wc_api.post(product_variation_batch % (data.get('id')), {'create': variants_to_create})
            except Exception as error:
    ...
    def convert_weight_by_uom(self, weight, instance, import_process=False):
        ...
        product_weight_uom = self.env[product_template_ref]._get_weight_uom_id_from_ir_config_parameter()
    ...
    def export_woo_variants(self, variants, woo_tmpl_id, wcapi, woo_template):
        ...
        for woo_variants in variant_batches:
            try:
                variant_response = wcapi.post(product_variation_batch % woo_tmpl_id, {'create': woo_variants})
            except Exception as error:
...
class ProductProductEpt(models.Model):
    ...
    product_id = fields.Many2one(product_product_ref, required=True, ondelete="cascade")
    woo_template_id = fields.Many2one(woo_product_template_ept, required=True, ondelete="cascade")
    active = fields.Boolean(default=True)
    ...
    woo_image_ids = fields.One2many(woo_product_image_ref, "woo_variant_id")
