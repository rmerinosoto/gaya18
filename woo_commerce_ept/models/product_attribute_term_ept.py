...
class WooProductAttributeTermEpt(models.Model):
    ...
    attribute_id = fields.Many2one('product.attribute', 'Attribute', required=True, copy=False)
    attribute_value_id = fields.Many2one('product.attribute.value', 'Attribute Value', required=True, copy=False)
    woo_instance_id = fields.Many2one("woo.instance.ept", "Instance", required=True, ondelete='cascade')
